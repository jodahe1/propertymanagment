...
export class HotelRepository
  extends MikroOrmBaseRepository<Hotel>
  implements THotelRepository
{
  constructor(
  ...
  protected updateEntity(entity: HotelPersistenceEntity, updates: Hotel): void {
    if (updates.user_id)
      entity.user = this.entityManager.getReference(
    ...
    if (updates.contact_email !== undefined)
      entity.contact_email = updates.contact_email;
    if (updates.contact_phone !== undefined)
      entity.contact_phone = updates.contact_phone;
    if (updates.star_rating !== undefined)
      entity.star_rating = updates.star_rating;
    ...
    if (updates.check_in_instructions !== undefined)
      entity.check_in_instructions = updates.check_in_instructions;
    if (updates.legal_information !== undefined)
      entity.legal_information = updates.legal_information;
  }
}
